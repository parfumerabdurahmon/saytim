        ...
        <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
          <div className="flex items-center space-x-4 group cursor-pointer" onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
            <div className="w-10 h-10 bg-man-gold flex items-center justify-center relative overflow-hidden">
               <i className="fas fa-crown text-black text-xl relative z-10"></i>
            </div>
          ...
          <div className="flex items-center space-x-8">
            <div className="flex border border-white/10 overflow-hidden text-[9px] font-black">
              <button 
                ...
                className={`px-4 py-2 transition-colors ${lang === 'uz' ? 'bg-man-gold text-black' : 'text-gray-400 hover:text-white'}`}
              >UZ</button>
              <button 
                onClick={() => { setLang('ru'); window.dispatchEvent(new CustomEvent('langChange', { detail: 'ru' })); }}
                className={`px-4 py-2 transition-colors ${lang === 'ru' ? 'bg-man-gold text-black' : 'text-gray-400 hover:text-white'}`}
              >RU</button>
            </div>
          </div>
        </div>
      ...
      <footer id="contact" className="bg-[#000000] py-32 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <div className="flex flex-col items-center mb-12">
            ...
            <p className="text-gray-600 font-light text-xs tracking-widest uppercase max-w-md mx-auto">
